...
async def toggle_user_league(user_id, league_name):
    pool = await get_pool()
    async with pool.acquire() as conn:
        existing = await conn.fetchval(
            "select id from user_leagues where user_id=$1 and league_name=$2",
            user_id, league_name,
        )
        if existing:
            await conn.execute("delete from user_leagues where id=$1", existing)
            return False
        await conn.execute(
            "insert into user_leagues (user_id, league_name) values ($1, $2)",
            user_id, league_name,
        )
        return True


async def get_user_leagues(user_id):
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "select league_name from user_leagues where user_id=$1",
            user_id,
        )
    return [r["league_name"] for r in rows]


async def get_users_for_market(sport, market, league=None):
    """Users pinned to this sport/market. League filter rule:
    - User with NO league rows: receives all leagues (default).
    - User with SOME league rows: only receives signals from those leagues.
    """
    pool = await get_pool()
    async with pool.acquire() as conn:
        if league:
