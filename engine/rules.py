"""Signal rules. Two tiers:
  - tier='signal'  → strong, post the bet recommendation
  - tier='watch'   → interesting moment, informational only (no bet)
"""
...
def shots_burst(fixture, current_stats, history) -> Optional[Signal]:
    ...
    if shots_delta >= 3:
        return Signal(
            ...
            criteria={"minute": minute, "shots_delta": shots_delta,
                      "score": f"{home_goals}-{away_goals}",
                      "window_min": minute - base["minute"]},
        )
    if shots_delta == 2:
        return Signal(
            rule_name="shots_burst_watch", market="goals",
...
def corner_spam(fixture, current_stats, history) -> Optional[Signal]:
    minute = fixture.get("fixture", {}).get("status", {}).get("elapsed") or 0
...
def late_push(fixture, current_stats, history) -> Optional[Signal]:
    minute = fixture.get("fixture", {}).get("status", {}).get("elapsed") or 0
    ...
    if dangerous >= 40 and possession >= 55:
        return Signal(
            ...
            confidence=3,
            criteria={"minute": minute, "trailing_team": trailing_id,
                      "dangerous_attacks": dangerous, "possession": possession},
        )
    if dangerous >= 30 and possession >= 52:
        return Signal(
            rule_name="late_push_watch", market="goals",
            suggested_bet="Watch tier · not a bet recommendation",
            confidence=2, tier="watch",
            criteria={"minute": minute, "trailing_team": trailing_id,
                      "dangerous_attacks": dangerous, "possession": possession},
